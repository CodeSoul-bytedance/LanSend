        ...
        handleBackendMessage(message) {
            ...
            switch (message.type) {
                ...
                case "ConnectedToDevice":
                    // 更新设备连接状态
                    const connectedDevice = this.devices.find(d => d.device_id === message.device_id);
                    if (connectedDevice) {
                ...
                case "RecipientAccepted":
                    ...
                    const transferIdx = this.transfers.findIndex(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                    if (transferIdx !== -1) {
                ...
                case "RecipientDeclined":
                    // 更新传输状态为已拒绝
                    const declinedTransferIdx = this.transfers.findIndex(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                ...
                case "FileSendingProgress":
                    // 找到对应的传输任务
                    const sendingTransfer = this.transfers.find(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                    if (sendingTransfer) {
                        // 更新文件进度
                        const fileIdx = sendingTransfer.files.findIndex(f => f.name === message.file_name);
                        if (fileIdx !== -1) {
                            sendingTransfer.files[fileIdx].progress = message.progress;
                        }
                        
                        // 计算传输的总大小
                        const transferredSize = sendingTransfer.files.reduce((sum, file) => 
                            sum + ((file.progress || 0) / 100) * (file.size || 0), 0);
                ...
                case "FileSendingCompleted":
                    // 找到对应的传输任务
                    const completedSendingTransfer = this.transfers.find(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                    if (completedSendingTransfer) {
                        // 更新文件状态
                        const fileIdx = completedSendingTransfer.files.findIndex(f => f.name === message.file_name);
                        if (fileIdx !== -1) {
                            completedSendingTransfer.files[fileIdx].completed = true;
                            completedSendingTransfer.files[fileIdx].progress = 100;
                        }
                        
                        // 检查所有文件是否都已完成
                        const allCompleted = completedSendingTransfer.files.every(f => f.completed);
                        if (allCompleted) {
                            this.updateTransferStatus({
                                transfer_id: completedSendingTransfer.id,
                                status: "completed",
                                transferred_size: completedSendingTransfer.totalSize
                            });
                        } else {
                            // 计算已传输的总大小
                            const transferredSize = completedSendingTransfer.files.reduce((sum, file) => 
                                sum + ((file.progress || 0) / 100) * (file.size || 0), 0);
                ...
                case "AllSendingCompleted":
                    // 找到对应的传输任务
                    const allCompletedTransfer = this.transfers.find(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                ...
                case "FileReceivingProgress":
                    // 更新文件接收进度
                    const receivingTransfer = this.transfers.find(t => 
                        t.id === message.transfer_id || 
                        (t.files.some(f => f.name === message.file_name) && t.status === "in_progress"));
                    
                    if (receivingTransfer) {
                        // 更新文件进度
                        const fileIdx = receivingTransfer.files.findIndex(f => f.name === message.file_name);
                        if (fileIdx !== -1) {
                            receivingTransfer.files[fileIdx].progress = message.progress;
                        }
                        
                        // 计算传输的总大小
                        const transferredSize = receivingTransfer.files.reduce((sum, file) => 
                            sum + ((file.progress || 0) / 100) * (file.size || 0), 0);
                ...
                case "FileReceivingCompleted":
                    // 更新单个文件接收完成状态
                    const completedReceivingTransfer = this.transfers.find(t => 
                        t.id === message.transfer_id || 
                        (t.files.some(f => f.name === message.file_name) && t.status === "in_progress"));
                    
                    if (completedReceivingTransfer) {
                        // 更新文件状态
                        const fileIdx = completedReceivingTransfer.files.findIndex(f => f.name === message.file_name);
                        if (fileIdx !== -1) {
                            completedReceivingTransfer.files[fileIdx].completed = true;
                            completedReceivingTransfer.files[fileIdx].progress = 100;
                        }
                        
                        // 检查所有文件是否都已完成
                        const allCompleted = completedReceivingTransfer.files.every(f => f.completed);
                        if (allCompleted) {
                            this.updateTransferStatus({
                                transfer_id: completedReceivingTransfer.id,
                                status: "completed",
                                transferred_size: completedReceivingTransfer.totalSize
                            });
                        } else {
                            // 计算已传输的总大小
                            const transferredSize = completedReceivingTransfer.files.reduce((sum, file) => 
                                sum + ((file.progress || 0) / 100) * (file.size || 0), 0);
                ...
                case "AllReceivingCompleted":
                    // 更新所有文件接收完成状态
                    // 找到当前正在接收的传输
                    const allReceivingCompletedTransfer = this.transfers.find(t => 
                        t.status === "in_progress" && t.sourceDevice);
                    ...
                    if (allReceivingCompletedTransfer) {
                        ...
                        this.updateTransferStatus({
                            ...
                            status: "completed",
                            transferred_size: allReceivingCompletedTransfer.totalSize
                        });
                    }
                    break;
                case "SendingCancelledByReceiver":
                    // 处理接收者取消发送
                    const cancelledByReceiverTransfer = this.transfers.find(t => 
                        t.targetDevice && t.targetDevice.device_id === message.device_id);
                ...
                case "ReceivingCancelledBySender":
                    // 处理发送者取消接收
                    // 找到当前正在接收的传输
                    const cancelledBySenderTransfer = this.transfers.find(t => 
                        t.status === "in_progress" && t.sourceDevice);
