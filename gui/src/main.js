...
// 定时扫描设备
let deviceScanInterval = null;
const DEVICE_SCAN_INTERVAL_MS = 60000; // Scan every 60 seconds
...
// Centralized function to update GUI and manage scanning
function updateAndReportOverallBackendStatus(reasonDetails) {
    ...
    if (newOverallStatus) {
        if (deviceScanInterval) clearInterval(deviceScanInterval);
        console.log(
            "All conditions met. Starting/Restarting periodic device scanning."
        );
        triggerDeviceScan(); // Initial scan
        deviceScanInterval = setInterval(
            triggerDeviceScan,
        ...
        if (deviceScanInterval) {
            ...
            console.log(
                `Conditions for periodic scan not met. Periodic scanning stopped/not started.`
            );
        }
    }
}
    ...
    const stdinServer = createPipeServer(
        ...
        (socket) => {
            ...
            socket.on("error", (err) => {
                ...
                individualStdinConnected = false;
                updatePipeConnectionStatus({
                    message: `后端STDIN管道socket错误: ${err.message}`,
                });
            ...
            socket.on("close", () => {
                ...
                individualStdinConnected = false;
                updatePipeConnectionStatus({ message: "后端STDIN管道已关闭" });
                backendStdinSocket = null;
            });
        },
        (err) => {
            ...
            individualStdinConnected = false;
            updatePipeConnectionStatus({
                message: `STDIN管道服务器错误: ${err.message}`,
            });
        }
    );
    ...
    const stdoutServer = createPipeServer(
        ...
        (socket) => {
            ...
            socket.on("error", (err) => {
                ...
                individualStdoutConnected = false;
                updatePipeConnectionStatus({
                    message: `后端STDOUT管道socket错误: ${err.message}`,
                });
            ...
            socket.on("close", () => {
                ...
                individualStdoutConnected = false;
                updatePipeConnectionStatus({ message: "后端STDOUT管道已关闭" });
                backendStdoutSocket = null;
            });
        },
        (err) => {
            ...
            individualStdoutConnected = false;
            updatePipeConnectionStatus({
                message: `STDOUT管道服务器错误: ${err.message}`,
            });
        }
    );
...
async function triggerDeviceScan() {
    ...
    if (
        ...
        console.log("Skipping device scan: conditions not met.");
        return;
    }
    console.log("Requesting device scan from backend...");
    try {
        await sendToBackend({
            type: "scan_devices",
            timestamp: new Date().toISOString(),
        });
        console.log("Device scan request sent.");
    } catch (error) {
        console.error("Error triggering device scan:", error);
    }
}
